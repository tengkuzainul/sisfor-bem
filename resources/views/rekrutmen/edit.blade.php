...
@section('page-header')
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Edit Rekrutmen</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $rekrutmen->judul }}</p>
    </div>
@endsection

@section('content')
    <div class="mx-auto max-w-2xl">
        <form action="{{ route('rekrutmen.update', $rekrutmen) }}" method="POST" enctype="multipart/form-data"
              class="glass-card p-6">
            @csrf
            @method('PUT')

            @include('rekrutmen._form')

            <div class="mt-6 flex items-center justify-end gap-3 border-t border-gray-100 pt-6 dark:border-gray-800">
                <a href="{{ route('rekrutmen.index') }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                    Batal
                </a>
