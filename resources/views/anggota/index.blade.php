        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                ...
                @if($activeKepengurusan)
                    <span class="inline-flex items-center gap-1 rounded-full bg-success-50 px-2 py-0.5 text-xs font-medium text-success-700 dark:bg-success-900/30 dark:text-success-400">
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <th @click="sort('nim')" class="cursor-pointer select-none px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 transition hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                            <span class="flex items-center gap-1">NIM <x-datatable.sort-icon column="nim" /></span>
                        </th>
                        <th @click="sort('prodi')" class="cursor-pointer select-none px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 transition hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                            <span class="flex items-center gap-1">Prodi <x-datatable.sort-icon column="prodi" /></span>
                        </th>
                        <th class="px-6 py-3.5 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Jabatan</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    <template x-for="(item, index) in data" :key="item.id">
                        <tr class="transition hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                            ...
                            <td class="px-6 py-3.5 font-mono text-xs text-gray-600 dark:text-gray-400" x-text="item.nim"></td>
                            <td class="px-6 py-3.5 text-gray-600 dark:text-gray-400" x-text="item.prodi || '-'"></td>
                            <td class="px-6 py-3.5">
                                <span class="text-gray-900 dark:text-white" x-text="item.active_keanggotaan?.jabatan?.nama || '-'"></span>
                            </td>
                            <td class="px-6 py-3.5">
                                <template x-if="item.active_keanggotaan?.departemen">
                                    <span class="inline-flex items-center rounded-full bg-accent-50 px-2.5 py-0.5 text-xs font-medium text-accent-700 dark:bg-accent-900/30 dark:text-accent-400"
                                ...
                                <template x-if="item.active_keanggotaan && !item.active_keanggotaan.departemen">
                                    <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">BPH</span>
                                </template>
                            ...
                            <td class="px-6 py-3.5">
                                <div class="flex items-center gap-1">
                                    <a :href="'{{ url('anggota') }}/' + item.id + '/edit'" class="rounded-lg p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-800 dark:hover:text-gray-300">
                                        <x-heroicon-o-pencil-square class="h-4 w-4" />
                                    </a>
                                    <button @click="deleteItem('{{ url('anggota') }}/' + item.id)" class="rounded-lg p-1.5 text-gray-400 transition hover:bg-danger-50 hover:text-danger-600 dark:hover:bg-danger-900/30 dark:hover:text-danger-400">
