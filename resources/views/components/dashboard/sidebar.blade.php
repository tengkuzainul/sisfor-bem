...
@php
    ...
    */
    $menu = [
        ...
        [
            'label' => 'Organisasi',
            'roles' => ['administrator', 'kaprodi', 'pembina'],
            'items' => [
                [
                    'label'   => 'Kepengurusan',
...
>
    ...
    <div class="flex h-16 items-center gap-3 border-b border-gray-200/40 px-5 dark:border-white/[0.08]">
        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-primary-500 to-primary-700 shadow-sm">
        ...
        <div class="flex flex-col">
            ...
            <span class="text-[10px] font-medium uppercase tracking-wider text-gray-400 dark:text-gray-500">Management System</span>
        </div>
        {{-- Close button mobile --}}
        <button @click="sidebarOpen = false" class="ml-auto rounded-lg p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600 lg:hidden dark:hover:bg-gray-800">
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4 scrollbar-thin">
        @foreach($menu as $section)
            ...
            {{-- Section Header --}}
            <p class="mb-2 {{ !$loop->first ? 'mt-6' : '' }} px-3 text-[10px] font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500">
                {{ $section['label'] }}
            </p>
        ...
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open"
                ...
                <div class="flex-1 text-left">
                    <p class="truncate text-sm font-medium text-gray-900 dark:text-white">{{ $user->name ?? 'Admin' }}</p>
