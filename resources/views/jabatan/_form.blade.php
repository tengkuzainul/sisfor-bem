...
<div class="space-y-5">
    {{-- Nama --}}
    <div>
        <label for="nama" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Jabatan <span
    ...
    {{-- Level --}}
    <div>
        <label for="level" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Level Hierarki <span
                class="text-danger-500">*</span></label>
        <input type="number" name="level" id="level" value="{{ old('level', $jabatan->level ?? 0) }}"
            min="0"
        ...
        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Semakin kecil angka, semakin tinggi posisinya (0 =
            tertinggi).</p>
        @error('level')
            <p class="mt-1 text-xs text-danger-600">{{ $message }}</p>
        @enderror
    ...
    <div>
        ...
        <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Deskripsi singkat jabatan..."
